                ...
                <div class="flex flex-col gap-6">
                    ...
                    <div class="flex flex-col gap-6">
                        <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
                            ...
                            <div class="space-y-6">
                                <div>
                                    <label class="mb-3 block text-sm font-medium text-gray-700 dark:text-gray-400">Destination Name</label>
                                    <input type="text" wire:model="name" placeholder="Enter destination name" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800 transition-colors">
                                </div>
                                
                                <div>
                                    <label class="mb-3 block text-sm font-medium text-gray-700 dark:text-gray-400">Location</label>
                                    <input type="text" wire:model="location" placeholder="e.g., South Halmahera, North Maluku" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800 transition-colors">
                                </div>
                                ...
                                <div>
                                    ...
                                    <textarea wire:model="description" rows="5" placeholder="Detailed information about the destination..." class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800 transition-colors"></textarea>
                                </div>
                            </div>
                        </div>

                        <!-- Interactive Map Card -->
                        <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6" wire:ignore>
                            ...
                            <div class="relative">
                                <div x-data="destinationMap" x-init="initMap($refs.mapContainer)" class="w-full relative">
                    ...
                    <!-- Right Column (Media, Pricing, Category) -->
                    <div class="flex flex-col gap-6">
                        
                        <!-- Media Card -->
                        <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
                            <div class="flex items-center justify-between mb-5">
                                <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Media</h3>
                                <span style="font-size: 12px; color: #94a3b8;">Maks. 10 gambar</span>
                            </div>
                            
                            @error('images') <p style="color: #ef4444; font-size: 13px; margin-bottom: 8px;">{{ $message }}</p> @enderror
                            @error('images.*') <p style="color: #ef4444; font-size: 13px; margin-bottom: 8px;">{{ $message }}</p> @enderror
                            ...
                            @if(count($existingImages) > 0)
                                ...
                                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-bottom: 12px;">
                                    @foreach($existingImages as $idx => $media)
                                        <div style="position: relative; border-radius: 8px; overflow: hidden; border: 1px solid #374151; aspect-ratio: 1/1;">
                                            ...
                                            <button type="button" wire:click="removeExistingImage({{ $media['id'] }})"
                                                    style="position: absolute; top: 4px; right: 4px; width: 22px; height: 22px; border-radius: 50%; background: rgba(239,68,68,0.9); color: white; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 14px; line-height: 1;">
                                                ×
                                            </button>
                                        </div>
                                    @endforeach
                            ...
                            @if(count($images) > 0)
                                <label style="display: block; font-size: 13px; font-weight: 500; color: #94a3b8; margin-bottom: 8px;">Gambar Baru ({{ count($images) }})</label>
                                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-bottom: 12px;">
                                    @foreach($images as $idx => $img)
                                        <div style="position: relative; border-radius: 8px; overflow: hidden; border: 1px solid #374151; aspect-ratio: 1/1;">
                                            <img src="{{ $img->temporaryUrl() }}" style="width: 100%; height: 100%; object-fit: cover;">
                            ...
                            <div class="relative block w-full appearance-none rounded-lg border border-dashed border-gray-300 bg-transparent px-4 py-4 text-center dark:border-gray-700 hover:border-brand-500 hover:bg-brand-50/50 dark:hover:bg-brand-500/5 transition-colors cursor-pointer">
                                <input type="file" wire:model="images" multiple class="absolute inset-0 z-50 m-0 h-full w-full cursor-pointer p-0 opacity-0 outline-none" accept="image/*">
                                <div class="flex flex-col items-center justify-center space-y-2 text-gray-500 dark:text-gray-400">
                                    ...
                                    <p class="text-xs">PNG, JPG atau GIF (maks. 20MB/file, maks. 10 file)</p>
                                </div>
                            </div>
                            <div wire:loading wire:target="images" style="color: #6366f1; font-size: 13px; margin-top: 8px; font-weight: 500;">Mengupload gambar...</div>
                        </div>

                        <!-- Organization Card -->
                        <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
                            <h3 class="mb-5 text-lg font-semibold text-gray-800 dark:text-white/90">Organization</h3>
                            
                            <div class="space-y-6">
                                <div>
                                    <label class="mb-3 block text-sm font-medium text-gray-700 dark:text-gray-400">Category</label>
                                    <div class="relative bg-transparent">
                                        <select wire:model="category" class="h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 pr-11 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800 transition-colors">
                                ...
                                <div x-data="{ 
                                    ...
                                    <label class="mb-3 block text-sm font-medium text-gray-700 dark:text-gray-400">Entry Fee (Rp)</label>
                                    <div class="relative">
                                ...
                                <div>
                                    <label class="mb-3 block text-sm font-medium text-gray-700 dark:text-gray-400">Facilities / What this place offers</label>
                                    <div class="grid grid-cols-2 gap-3">
                                        @foreach(['Swimming Area', 'Snorkeling', 'Private Cabanas', 'Local Culinary', 'Restrooms', 'Secure Parking', 'Photo Spots', 'Camping Ground', 'WIFI'] as $facility)
                                ...
                                <div>
                                    <label class="mb-3 block text-sm font-medium text-gray-700 dark:text-gray-400">Status</label>
                    ...
                    @forelse ($destinations as $destination)
                        <tr class="border-b border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                            <td class="py-4 px-5 whitespace-nowrap">
                                @if($destination->getFirstMediaUrl('default'))
                                    <img src="{{ $destination->getFirstMediaUrl('default') }}" class="w-12 h-12 rounded-lg object-cover">
                                @else
                            ...
                            <td class="py-4 px-5 whitespace-nowrap">
                                <div class="font-medium text-gray-800 dark:text-white/90">{{ $destination->name }}</div>
                                <div class="text-xs text-gray-500 dark:text-gray-400">Rp {{ number_format($destination->price, 0, ',', '.') }}</div>
                            </td>
                            <td class="py-4 px-5 whitespace-nowrap">
                            ...
                            <td class="py-4 px-5 whitespace-nowrap text-right">
                                <div class="flex items-center justify-end gap-3">
                                    <button wire:click="edit({{ $destination->id }})" class="text-gray-500 hover:text-brand-500 transition-colors">
                                    ...
                                    <button type="button" wire:click="confirmDelete({{ $destination->id }})" class="text-gray-500 hover:text-error-600 transition-colors">
                                        <svg class="fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
